...
struct Streams {
    ...
    static func inputStreams(_ resources: [(name: String, ext: String)]) -> [(url: URL, inputStream: InputStream)]? {
    ...
    static func readByte(_ inputStream: InputStream) -> UInt8 {
    ...
    static func writeBytes<T>(_ buffer: UnsafeMutablePointer<T>, count: Int, outputStream: OutputStream) {
        buffer.withMemoryRebound(to: UInt8.self, capacity: count) { (byteBuffer) -> Void in
            outputStream.write(byteBuffer, maxLength: count)
        }
    }
    
    static func readBoolean(_ inputStream : InputStream) -> Bool {
        return readByte(inputStream) == 1
    }
    
    static func readInt(_ inputStream : InputStream) -> Int {
    ...
    static func writeInt(_ int: Int, outputStream: OutputStream) {
    ...
    static func writeCharacter(_ character: Character, outputStream: OutputStream) {
    ...
    static func readFloat(_ inputStream : InputStream) -> Float {
        let buffer = UnsafeMutablePointer<UInt8>.allocate(capacity: integerSize)
        let read = inputStream.read(buffer, maxLength: integerSize)
    ...
    static func readDouble(_ inputStream : InputStream) -> Double {
    ...
    static func readByteArray(_ inputStream : InputStream) -> [UInt8] {
    ...
    static func readNullableByteArray(_ inputStream : InputStream) -> [UInt8]? {
    ...
    static func readIntArray(_ inputStream : InputStream) -> [Int] {
        let count = readInt(inputStream)
        var result = [Int]()
        result.reserveCapacity(count)
    ...
    static func readString(_ inputStream : InputStream) -> String {
    ...
    static func readNullableString(_ inputStream : InputStream) -> String? {
        if readBoolean(inputStream) {
    ...
    static func readColor(_ inputStream : InputStream) -> Color {
    ...
    static func readPoint(_ inputStream: InputStream) -> Spot {
        let x = readFloat(inputStream)
        let y = readFloat(inputStream)
        return Spot(x: x, y: y)
    }

    static func readFloatFromByteArray(_ bytes: [UInt8], atIndex start: Int) -> (float: Float, readCount: Int) {
    ...
    static func readStringFromByteArray(_ bytes: [UInt8], atIndex start: Int) -> (string: String, readCount: Int) {
        let pointer = UnsafeMutablePointer<UInt8>.allocate(capacity: 4)
        pointer[0] = bytes[start + 0]
        pointer[1] = bytes[start + 1]
        pointer[2] = bytes[start + 2]
